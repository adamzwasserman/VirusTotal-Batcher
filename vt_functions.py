...
@riprova.retry(backoff=riprova.ExponentialBackOff(interval=0.5, factor=0.5, max_interval=60, max_elapsed=900, multiplier=1.5),on_retry=on_retry)
def lookup_domains(vt_lookup_value):
    ...
    try:
        ...
        try:
            if response_json['resolutions']:
                for i in response_json['resolutions']:
                    ...
                    list_to_return.append((vt_lookup_value, vt_status, vt_ip, vt_date, vt_subdomain, vt_url, vt_score))
                vt_ip = ''
                vt_date = ''
        except KeyError:
            pass
        ...
        list_to_return = [(vt_lookup_value, vt_status, vt_date, vt_hostname, vt_url, vt_score)]
...
def lookup_ips(passed_value):
    lookup_list = ipaddress.ip_network(passed_value, strict=False)
    list_to_return = []
    for vt_lookup_value in lookup_list:
        print('Looking up', vt_lookup_value, end='..')
        list_to_return.extend(lookup_slash_ips(vt_lookup_value)) #  Call subroutine
    return list_to_return

#  Subroutine to step through "slash" IP networks
@riprova.retry(backoff=riprova.ExponentialBackOff(interval=2.5, factor=0.5, max_interval=60, max_elapsed=cfg.max_wait, multiplier=1.5),on_retry=on_retry)
