# BreakPoint Labs' dns-over-https modified to use urlquick instead of requests.
# This is because requests is not MIT license and urlquick is.

import random
import urlquick  # Does not need to be installed with pip. The file is included with this distro.
...
class SecureDNS(object):
    ...
    def resolve(self, hostname):
        ...
        r = urlquick.get(self.url, params=self.params)
        if r.status_code == 200:
            ...
            elif response['Status'] == SERVFAIL:
                return (['SERVFAIL'])

            elif response['Status'] == NXDOMAIN:
                return (['NXDOMAIN'])
